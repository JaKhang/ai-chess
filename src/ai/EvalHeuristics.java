...
public class EvalHeuristics implements Heuristics{
    ...
    public int get(Board state) {
        return calcAlliance(state.getCurrent(), state) - calcAlliance(Alliance.next(state.getCurrent()),state);
    }

    private int calcAlliance(Alliance alliance, Board board){
        int score = 0;
        for (var square : board){
            if (square.isNotBlank() && square.getPieces().is(alliance)){
